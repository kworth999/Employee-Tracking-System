...
newDepartment(connection, doMore) {
    inquirer.prompt(
        [{
            name: 'deptName',
            type: 'input',
            message: 'Enter a new depaqrtment name: ',
            validate: function (input) {
                if (input === null) {
                    return 'Please enter a number'
                };
    ...
    .then((answers) => {
        ...
        return connection.query(query, [answers.deptName], (err, res) => {
            if (err) throw err;
            console.log('New deparetment added successfully!');
            console.table(res);
            doMore();
        });
    });
